...
It also inspected every non-perfect result across six local validation runs.
Model aliases and generated result files are intentionally omitted from this
public methodology report because they describe one deployment rather than the
benchmark itself.
...
## Findings from validation runs

- Output exhaustion without final content occurred in both finance and
  reasoning tasks. Those cases are now recorded as `truncated`, not as ordinary
  quality failures.
- Normally terminated wrong answers remain quality failures; the result-state
  layer does not excuse incorrect model output.
- Small category panels can exaggerate task-level differences between models.
  Additional repository tasks improve resolution without encoding a preferred
  model ordering.
- Tool protocol and task-success dimensions are now reported separately so a
  high protocol score is not described as broad agentic success.
- Normally terminated, component-graded structured responses are distinguished
  from transport and parser failures.
- Validation responses and tool-call objects were parseable, but the legacy
  harness could not have distinguished every infrastructure failure. The new
  explicit result states close that gap.
