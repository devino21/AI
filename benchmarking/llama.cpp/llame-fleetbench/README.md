...
Quality and throughput benchmark for OpenAI-compatible local model endpoints.
With llama-swap, one command can walk every configured model and let the server
handle model changes. It runs nine test categories and emits a comparison table
with quality scores and tokens/sec per model. For example:

```
model                              tools  coding  reason  longctx  overall   pp t/s   tg t/s
--------------------------------------------------------------------------------------------
model-a                              92%     85%    100%      88%      91%      180      18.6
model-b                              83%     80%     83%     100%      86%      240      21.2
model-c                              75%     72%     83%      75%      76%      320      24.8
```
...
"how do locally served quants and runtime settings behave on representative
work" — tool calls, small coding tasks, and retrieval deep in context — in one
resumable sweep. That's the niche.
...
tie-breakers that matter for local deployments where wall-clock time matters.
...
11. [FAQ](#faq)
12. [Security and publication](#security-and-publication)
13. [Caveats](#caveats)
...
FleetBench only talks to one OpenAI-compatible `/v1/chat/completions`
endpoint. When that endpoint is llama-swap, changing the `model` field between
runs triggers the swap—no SSH, config rewrites, or llama-server restarts.
...
python fleetbench.py --selftest  # scorer/ground-truth checks; no server needed
```

Expected output ends with `All self-tests passed.` Then edit
`fleetbench.yaml`. Replace the public `example-model` entry and confirm:

- `base_url` is the OpenAI-compatible API root ending in `/v1`.
- Every model `name` is an id accepted by that endpoint. With llama-swap, it
  must exactly match a model key in the llama-swap configuration.
- Every model `ctx` reflects the context size actually served.

The checked-in file uses localhost and contains no deployment-specific model
inventory. Never commit a real API key; use a private copy of the configuration
when authentication is required.

Optional dry run against the included mock server before using an inference
endpoint:

```bash
python3 mock_server.py &
python3 fleetbench.py --config fleetbench.mock.yaml
```
...
| key | default | meaning |
|---|---|---|
| `base_url` | — | OpenAI-compatible endpoint, e.g. `http://127.0.0.1:8080/v1` |
...
python3 fleetbench.py --config fleetbench.yaml --models model-a --categories reasoning

# subset of models / categories
python3 fleetbench.py --config fleetbench.yaml --models model-a,model-b --categories tools,longctx
...
they compose: `--models model-a --tier hard --categories coding,reasoning`
...
> **If the core tier saturates:** run `--tier frontier` first on one fast model.
> It exercises stateful multi-step tools, recovery and verification,
...
**Long fleet run** (cold loads and reasoning-heavy models can take hours):
...
same model are just two entries — define `model-a-q4` and `model-a-q5` in
...
lm_eval --model local-chat-completions \
  ...
  --model_args model=model-a,base_url=http://127.0.0.1:8080/v1/chat/completions,num_concurrent=1,max_retries=3,tokenized_requests=False
```
...
check whether another client or monitoring process was hitting the
...
Runtime varies substantially with model size, reasoning behavior, endpoint
speed, context depths, and cold-load time. A slow reasoning model with 64k
prefills can take hours. Start with `--tier core --models <fast-one>` to verify
the configuration before committing to a full fleet run.
...
- Coding tasks execute model-generated Python. The runner uses temporary
  directories and Python isolation flags, but it is **not an OS sandbox**. Run
  it only against models or endpoints you trust; use a locked-down container or
  VM for untrusted output.
- Result files contain full model responses and can include model identifiers,
  server metadata, quantization details, local model paths, and tool traces.
  Review them before sharing. Generated results, logs, verification artifacts,
  virtual environments, and Python bytecode are excluded by `.gitignore`.
- Keep authenticated endpoint credentials outside tracked configuration. The
  public starter uses `api_key: none` and a loopback URL.
- The HTML dashboard parses selected files locally in the browser; it does not
  upload result data.
