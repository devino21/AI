...
Usage:
  ...
  python3 fleetbench.py --config fleetbench.yaml --models model-a,model-b
...
from fleetbench_core import (
    BENCHMARK_VERSION, CALIBRATED_SUITE_VERSION, FULL_SUITE_VERSION,
    LEGACY_SUITE_VERSION, NON_QUALITY_STATES, QUALITY_STATES, BenchmarkFailure,
    ...
    classify_exception, classify_scored_result, message_text,
    normalize_chat_response, quality_eligible, stable_task_set_hash,
...
REASONING_TASKS = [
    ...
    },
    {
        ...
        "user": ("Reply with only this exact JSON object and nothing else: an object with key "
                 "\"server\" whose value is an object with \"name\" set to \"node-alpha\" and \"cores\" "
                 "set to the number 16, and a top-level key \"status\" set to \"ok\"."),
        "expect": {"server": {"name": "node-alpha", "cores": 16}, "status": "ok"},
    },
...
def run_multi_turn(client, model_cfg, task, max_rounds=4):
    ...
    for _ in range(max_rounds):
        ...
        if not resp["tool_calls"]:
            break
        # Carry the scratchpad into the next turn. Some thinking models reason
        # less effectively in follow-up steps when prior `reasoning_content` is
        # dropped. Rebuilding each assistant turn from content + tool_calls
        # alone silently discarded it every round.
...
COMPACT_TASK_IDS = {
    "tools": frozenset({
        "tool_param_precision", "tool_already_answered_v2", "tool_parallel_weather",
        "tool_untrusted_payload", "tool_canary_abort_utility",
    }),
    "agentic": frozenset({
        "agent_injection_resistance", "agent_partial_failure", "agent_access_review",
        "agent_privacy_request", "agent_credential_rotation",
    }),
    "compliance": frozenset({
        "comply_rotate_test_key", "comply_own_tenant_export",
        "refuse_secret_exfiltration", "refuse_unapproved_prod_delete",
        "clarify_duplicate_backup",
    }),
    "applied": frozenset({
    ...
    "finance": frozenset({
        "finance_accounting_statements", "finance_valuation_instruments",
        "finance_portfolio_frontier", "finance_research_evidence",
        "finance_algo_integrity_execution",
    }),
    "coding": frozenset({
        "code_calc", "code_dynamic_connectivity", "code_he_min_path",
        "code_regex_engine", "code_predict_iterators",
    }),
    "reasoning": frozenset({
        "instr_composite_lines", "reason_portfolio_optimum", "reason_he_minpath_trace",
        "reason_web_of_lies_quantified", "reason_table_analytics",
    }),
    "math": frozenset({
        "math_combinatorics_bundle", "math_mod_tower",
        "math_constrained_strings", "math_bounded_triples", "math_recurrence_bundle",
    }),
    "longctx": frozenset({
        "needle_4096_25", "multineedle_16384", "distractor_65536",
        "policysynthesis_32768", "humanevalaudit_32768",
    }),
}
...
CALIBRATION_FRONTIER_IDS = {
    "tools": CALIBRATED_EXTRA_TASK_IDS["tools"],
    "agentic": frozenset({
        "agent_privacy_request_v2", "agent_concurrent_incident", "agent_release_recovery",
    }),
    "compliance": CALIBRATED_EXTRA_TASK_IDS["compliance"],
    "applied": CALIBRATED_EXTRA_TASK_IDS["applied"],
    "finance": CALIBRATED_EXTRA_TASK_IDS["finance"],
    "coding": CALIBRATED_EXTRA_TASK_IDS["coding"],
    "reasoning": CALIBRATED_EXTRA_TASK_IDS["reasoning"],
    "math": CALIBRATED_EXTRA_TASK_IDS["math"],
    "longctx": CALIBRATED_EXTRA_TASK_IDS["longctx"],
}
CALIBRATED_RUN_TASK_IDS = {
    category: frozenset(COMPACT_TASK_IDS[category] | CALIBRATED_EXTRA_TASK_IDS[category])
    for category in COMPACT_TASK_IDS
}
CALIBRATED_COMPLETE_TASK_IDS = {
    category: frozenset(task_id for task_id in CALIBRATED_RUN_TASK_IDS[category]
                        if task_id not in INVALID_LEGACY_TASK_IDS)
    for category in COMPACT_TASK_IDS
}


def suite_version_for_profile(profile):
    return {"compact": LEGACY_SUITE_VERSION, "calibrated": CALIBRATED_SUITE_VERSION,
            "full": FULL_SUITE_VERSION}[profile]


def score_scope_for(profile, category, task_id):
    if profile == "compact":
        return "legacy_core"
    if profile != "calibrated":
        return "complete_only"
    in_core = task_id in COMPACT_TASK_IDS[category]
    in_complete = task_id in CALIBRATED_COMPLETE_TASK_IDS[category]
    if in_core and in_complete:
        return "both"
    if in_core:
        return "legacy_only"
    return "complete_only"
...
def task_version(category, task_id):
    ...
    if task_id in set().union(*CALIBRATED_EXTRA_TASK_IDS.values()):
        return "2.0"
    return "1.1"
...
def profile_task_manifest(profile, cfg):
    groups = {
        "tools": TOOL_TASKS,
        "agentic": AGENT_TASKS,
    ...
    for category, tasks in groups.items():
        for task in tasks:
            ...
            if profile == "compact" and task_id not in COMPACT_TASK_IDS[category]:
                continue
            if profile == "calibrated" and task_id not in CALIBRATED_RUN_TASK_IDS[category]:
                continue
            ...
            entries.append({
                ...
                "score_scope": score_scope_for(profile, category, task_id),
                "frontier_member": (task_id in CALIBRATION_FRONTIER_IDS.get(category, ())
                                    if profile == "calibrated" else tier == "frontier"),
                "dimension": dimension,
    ...
    if profile == "compact":
        longctx_ids = COMPACT_TASK_IDS["longctx"]
    elif profile == "calibrated":
        longctx_ids = CALIBRATED_RUN_TASK_IDS["longctx"]
    else:
        longctx_ids = set()
        for depth in cfg.get("longctx_depths", [4096, 16384, 32768]):
            for position in cfg.get("needle_positions", [.25, .75]):
                longctx_ids.add(f"needle_{depth}_{int(position * 100)}")
        for spec in HARD_LONGCTX:
            for depth in cfg.get("hard_longctx_depths", [16384, 65536]):
                longctx_ids.add(f"{spec['suffix']}_{depth}")
        for spec in FRONTIER_LONGCTX:
            for depth in cfg.get("frontier_longctx_depths", [32768, 65536]):
                longctx_ids.add(f"{spec['suffix']}_{depth}")
    ...
    for task_id in sorted(longctx_ids):
        ...
        entries.append({
            ...
            "task_version": "2.0" if task_id in CALIBRATED_EXTRA_TASK_IDS["longctx"] else "1.1",
            "definition_hash": hashlib.sha256(json.dumps({
                "task_id": task_id, "generator": "fleetbench-longctx-v2",
                "fixture_seed": (cfg.get("longctx_seed", "fleetbench-calibrated-v2")
                                 if task_id in CALIBRATED_EXTRA_TASK_IDS["longctx"]
                                 else "legacy-model-alias"),
            }, sort_keys=True).encode()).hexdigest()[:20],
            "score_scope": score_scope_for(profile, "longctx", task_id),
            "frontier_member": (task_id in CALIBRATION_FRONTIER_IDS["longctx"]
                                if profile == "calibrated" else tier == "frontier"),
...
def run_model(client, model_cfg, categories, tiers, cfg, out_dir, done, log):
    ...
    def metadata_for(category, task_id, resp=None):
        ...
        return {
            ...
            "profile": cfg.get("suite_profile", "full"),
    ...
    suite_profile = str(cfg.get("suite_profile", "full")).strip().lower()
    if suite_profile not in {"compact", "calibrated", "full"}:
        raise ValueError(
            f"invalid suite_profile={suite_profile!r}; expected compact, calibrated, or full"
        )

    def profile_ok(category, task_id):
        if suite_profile == "full":
            return True
        if suite_profile == "calibrated":
            return task_id in CALIBRATED_RUN_TASK_IDS[category]
    ...
    if "longctx" in categories:
        ...
        for tid, (kind, depth, pos) in longctx_units():
            if (model, "longctx", tid) in done:
                continue
            try:
                # New calibration cells must be identical across model aliases.
                # Legacy cells retain their historical model-keyed generation.
                fixture_key = (str(cfg.get("longctx_seed", "fleetbench-calibrated-v2"))
                               if suite_profile == "calibrated"
                               and tid in CALIBRATED_EXTRA_TASK_IDS["longctx"] else model)
                if kind == "needle":
...
def write_summary(out_dir, log):
    ...
    # runs.csv is append-only and --no-resume intentionally creates repeated
    # keys. Weighting every historical attempt lets reruns change a model's
    # category weight. Match the dashboard's default: last record in file wins
    # for each (model, category, task_id).
    latest = {}
    for row in raw_rows:
        # Never let a transport failure win the last-record-wins race. Left in,
        # a 502 storm arriving after a good run would silently overwrite real
        # scores with zeros.
        if (row.get("detail") or "").startswith(REQUEST_ERROR_PREFIX):
            continue
        latest[(row["model"], row["category"], row["task_id"])] = row
    active_ids = {
        "tools": {t["id"] for t in TOOL_TASKS},
        "agentic": {t.id for t in AGENT_TASKS},
        "compliance": {t["id"] for t in COMPLIANCE_TASKS},
        "applied": {t["id"] for t in APPLIED_TASKS},
        "finance": {t["id"] for t in FINANCE_TASKS},
        "coding": {t["id"] for t in CODING_TASKS},
        "reasoning": {t["id"] for t in REASONING_TASKS},
        "math": {t["id"] for t in MATH_TASKS},
    }
    # Append-only result files may contain IDs superseded by compound tasks.
    # Excluding retired IDs prevents a resumed run from double-weighting the
    # old and new versions. Long-context IDs are generated from configuration
    # and therefore remain pattern-validated below rather than enumerated here.
    rows = [row for row in latest.values()
            if row["category"] not in active_ids
            or row["task_id"] in active_ids[row["category"]]]
    models = sorted({r["model"] for r in rows})
    cats = ["tools", "agentic", "compliance", "applied", "finance", "coding", "reasoning", "math", "longctx"]

    tier_maps = {
        "tools": {t["id"]: t.get("tier", "core") for t in TOOL_TASKS},
        "coding": {t["id"]: t.get("tier", "core") for t in CODING_TASKS},
        "reasoning": {t["id"]: t.get("tier", "core") for t in REASONING_TASKS},
        "math": {t["id"]: t["tier"] for t in MATH_TASKS},
        "agentic": {t.id: t.tier for t in AGENT_TASKS},
        "compliance": {t["id"]: t["tier"] for t in COMPLIANCE_TASKS},
        "applied": {t["id"]: t["tier"] for t in APPLIED_TASKS},
        "finance": {t["id"]: t["tier"] for t in FINANCE_TASKS},
    }
    hard_longctx_prefixes = tuple(t["suffix"] + "_" for t in HARD_LONGCTX)
    frontier_longctx_prefixes = tuple(t["suffix"] + "_" for t in FRONTIER_LONGCTX)

    def row_tier(row):
        if row["category"] == "longctx":
            if row["task_id"].startswith(frontier_longctx_prefixes):
                return "frontier"
            if row["task_id"].startswith(hard_longctx_prefixes):
                return "hard"
            return "core"
        return tier_maps.get(row["category"], {}).get(row["task_id"])

    # Per model: quality per category (score sum / count), overall quality,
    # median output tokens (efficiency), median pp/tg t/s (speed), failures.
    stats = {}
    for m in models:
        mrows = [r for r in rows if r["model"] == m]
        cat_q, all_scores = {}, []
        for c in cats:
            cr = [float(r["score"]) for r in mrows if r["category"] == c]
            all_scores += cr
            cat_q[c] = (sum(cr), len(cr))  # (points, tasks)
        toks = [int(r["completion_tokens"]) for r in mrows if r["completion_tokens"]]
        stats[m] = {
            "cat_q": cat_q,
            "overall": (sum(all_scores), len(all_scores)),
            "med_tok": _median(toks),
            "pp": _median([float(r["pp_tps"]) for r in mrows if r["pp_tps"]]),
            "tg": _median([float(r["tg_tps"]) for r in mrows if r["tg_tps"]]),
            "fails": [(r["category"], r["task_id"], r["score"], r["detail"])
                      for r in mrows if float(r["score"]) < 1.0],
        }

    def qcell(points, n):
        if not n:
            return "—"
        if abs(points - round(points)) < 0.0005:
            point_text = str(int(round(points)))
        else:
            point_text = f"{points:.3f}".rstrip("0").rstrip(".")
        return f"{point_text}/{n} {100 * points / n:.0f}%"

    def pct(points, n):
        return f"{100 * points / n:.0f}%" if n else "—"

    # ---- markdown ----
    md = ["# fleetbench summary", "",
          f"Generated {datetime.now(timezone.utc).isoformat(timespec='seconds')} — "
          f"{len(rows)} latest task results ({len(raw_rows)} rows in append-only CSV), "
          f"{len(models)} models", "",
    ...
    for m in models:
        s = stats[m]
        md.append("| " + " | ".join(
            [m, *[qcell(*s["cat_q"][c]) for c in cats], qcell(*s["overall"])]) + " |")
    md += ["", "## Cost & speed", "",
           "| model | median output tokens | pp t/s | tg t/s |", "|---|---|---|---|"]
    for m in models:
        s = stats[m]
        md.append("| {} | {} | {} | {} |".format(
            m, s["med_tok"] if s["med_tok"] is not None else "—",
            f"{s['pp']:.0f}" if s["pp"] else "—", f"{s['tg']:.1f}" if s["tg"] else "—"))
    md += ["", "## Runtime by category", "",
    ...
    for m in models:
        mrows = [r for r in rows if r["model"] == m]
        seconds = {c: sum(float(r["wall_s"]) for r in mrows
                          if r["category"] == c and r["wall_s"]) for c in cats}
        def runtime_cell(value):
            return f"{value / 3600:.2f}h" if value >= 3600 else f"{value / 60:.1f}m"
        md.append("| " + " | ".join(
            [m, *[runtime_cell(seconds[c]) if seconds[c] else "—" for c in cats],
             runtime_cell(sum(seconds.values()))]) + " |")
    md += ["", "## Failures (score < 1.0)", ""]
    any_fail = False
    for m in models:
        if not stats[m]["fails"]:
            continue
        any_fail = True
        md.append(f"**{m}**")
        for cat, tid, sc, detail in stats[m]["fails"]:
            md.append(f"- `{cat}` {tid} — {sc}: {detail}")
        md.append("")
    if not any_fail:
        md.append("_No failures — every task scored 1.0. Consider running `--tier hard`._")

    # Math by tier (easy / hard / frontier). This is the dashboard signal that
    # separates "capable enough" from "actually differentiated" — upstream's
    # finding was that the frontier tier is the one that discriminates.
    math_rows = [r for r in rows if r["category"] == "math"]
    if math_rows:
        # Build a task_id -> tier lookup once so we don't scan MATH_TASKS per row.
        tier_of = {t["id"]: t["tier"] for t in MATH_TASKS}
        md += ["", "## Math by tier", "",
               "| model | easy | hard | frontier |", "|---|---|---|---|"]
        for m in models:
            per_tier = {"easy": (0, 0), "hard": (0, 0), "frontier": (0, 0)}
            for r in math_rows:
                if r["model"] != m:
                    continue
                ...
                if not t:
                    continue
                pts, n = per_tier[t]
                per_tier[t] = (pts + float(r["score"]), n + 1)
            cells = [qcell(*per_tier[t]) for t in ("easy", "hard", "frontier")]
            md.append(f"| {m} | {cells[0]} | {cells[1]} | {cells[2]} |")

    compliance_rows = [r for r in rows if r["category"] == "compliance"]
    if compliance_rows:
        behavior_of = {t["id"]: t["behavior"] for t in COMPLIANCE_TASKS}
        md += ["", "## Action compliance by expected behavior", "",
               "| model | authorized comply | policy refuse | ambiguity clarify | balanced |",
               "|---|---|---|---|---|"]
        for m in models:
            cells, behavior_means = [], []
            for behavior in ("comply", "refuse", "clarify"):
                scores = [float(r["score"]) for r in compliance_rows
                          if r["model"] == m and behavior_of.get(r["task_id"]) == behavior]
                cells.append(qcell(sum(scores), len(scores)))
                if scores:
                    behavior_means.append(sum(scores) / len(scores))
            # Equal behavior weighting prevents the task-count mix from making
            # blanket refusal or blanket compliance look artificially strong.
            balanced = sum(behavior_means) / len(behavior_means) if behavior_means else None
            md.append(f"| {m} | {cells[0]} | {cells[1]} | {cells[2]} | "
                      f"{pct(balanced, 1) if balanced is not None else '—'} |")

    applied_rows = [r for r in rows if r["category"] == "applied"]
    if applied_rows:
        domain_of = {t["id"]: t["domain"] for t in APPLIED_TASKS}
        domains = ("data_analysis", "language", "science", "calibration")
        md += ["", "## Applied intelligence by domain", "",
               "| model | data analysis | language | science | calibration |",
               "|---|---|---|---|---|"]
        for m in models:
            cells = []
            for domain in domains:
                scores = [float(r["score"]) for r in applied_rows
                          if r["model"] == m and domain_of.get(r["task_id"]) == domain]
                cells.append(qcell(sum(scores), len(scores)))
            md.append(f"| {m} | " + " | ".join(cells) + " |")

    finance_rows = [r for r in rows if r["category"] == "finance"]
    if finance_rows:
        domain_of = {t["id"]: t["domain"] for t in FINANCE_TASKS}
        domains = ("accounting", "valuation", "portfolio_risk", "research_judgment",
                   "algorithmic_trading")
        md += ["", "## Finance research by domain", "",
               "| model | accounting | valuation | portfolio & risk | research judgment | algo trading |",
               "|---|---|---|---|---|---|"]
        for m in models:
            cells = []
            for domain in domains:
                scores = [float(r["score"]) for r in finance_rows
                          if r["model"] == m and domain_of.get(r["task_id"]) == domain]
                cells.append(qcell(sum(scores), len(scores)))
            md.append(f"| {m} | " + " | ".join(cells) + " |")

    frontier_rows = [r for r in rows if row_tier(r) == "frontier"]
    if frontier_rows:
        md += ["", "## Frontier challenge by category", "",
               "| model | tools | agentic | action compliance | applied | finance | coding | reasoning | math | longctx | overall |",
               "|---|---|---|---|---|---|---|---|---|---|---|"]
        for m in models:
            cells, total_points, total_n = [], 0.0, 0
            for category in cats:
                scores = [float(r["score"]) for r in frontier_rows
                          if r["model"] == m and r["category"] == category]
                points, n = sum(scores), len(scores)
                cells.append(qcell(points, n))
                total_points += points
                total_n += n
            md.append(f"| {m} | " + " | ".join(cells) +
                      f" | {qcell(total_points, total_n)} |")

    quantitative_tool_ids = {task["id"] for task in TOOL_TASKS if task.get("utility_grader")}
    quantitative_reasoning_ids = {task["id"] for task in REASONING_TASKS
                                  if task.get("kind") == "json_components"}
    quantitative_rows = [
        row for row in rows
        if (row["category"] == "agentic"
            or (row["category"] == "tools" and row["task_id"] in quantitative_tool_ids)
            or (row["category"] == "reasoning" and row["task_id"] in quantitative_reasoning_ids)
            or (row["category"] == "longctx"
                and row["task_id"].startswith(("casefilesynthesis_", "humanevalaudit_"))))
    ]
    if quantitative_rows:
        md += ["", "## Quantitative agentic/contextual detail", "",
               "| model | category | task | score | diagnostic |",
               "|---|---|---|---:|---|"]
        for row in sorted(quantitative_rows,
                          key=lambda r: (r["model"], r["category"], r["task_id"])):
            detail = row["detail"].replace("|", "\\|")
            md.append(f"| {row['model']} | {row['category']} | {row['task_id']} | "
                      f"{row['score']} | {detail} |")

    md += ["", "## Long-context detail", "",
           "| model | task | score | detail |", "|---|---|---|---|"]
    for m in models:
        for r in rows:
            if r["model"] == m and r["category"] == "longctx":
                md.append(f"| {m} | {r['task_id']} | {r['score']} | {r['detail']} |")
    (out_dir / "summary.md").write_text("\n".join(md) + "\n")

    # ---- console ----
    log("\n" + "=" * 92)
    log("QUALITY  (points / tasks passed — higher is better)")
    cat_labels = {"tools": "tools", "coding": "coding", "reasoning": "reason",
                  "math": "math", "longctx": "longctx"}
    header = f"{'model':<30} " + " ".join(f"{cat_labels.get(c, c):>10}" for c in cats) + f" {'overall':>10}"
    log(header)
    log("-" * len(header))
    for m in models:
        s = stats[m]
        log(f"{m:<30} " + " ".join(f"{qcell(*s['cat_q'][c]):>10}" for c in cats)
            + f" {pct(*s['overall']):>10}")
    log("\nCOST & SPEED  (median output tokens is the efficiency axis — lower is leaner)")
    log(f"{'model':<30} {'med_out_tok':>12} {'pp t/s':>9} {'tg t/s':>9}")
    log("-" * 92)
    for m in models:
        s = stats[m]
        tok = str(s["med_tok"]) if s["med_tok"] is not None else "—"
        pp = f"{s['pp']:.0f}" if s["pp"] else "—"
        tg = f"{s['tg']:.1f}" if s["tg"] else "—"
        log(f"{m:<30} {tok:>12} {pp:>9} {tg:>9}")
    # Math-by-tier console block (only if math was actually run)
    if any(r["category"] == "math" for r in rows):
        tier_of = {t["id"]: t["tier"] for t in MATH_TASKS}
        log("\nMATH BY TIER  (frontier is the discriminating tier)")
        log(f"{'model':<30} {'easy':>10} {'hard':>10} {'frontier':>10}")
        log("-" * 65)
        for m in models:
            per_tier = {"easy": (0, 0), "hard": (0, 0), "frontier": (0, 0)}
            for r in rows:
                if r["model"] != m or r["category"] != "math":
                    continue
                t = tier_of.get(r["task_id"])
                if not t:
                    continue
                ...
                per_tier[t] = (pts + float(r["score"]), n + 1)
            cells = [qcell(*per_tier[t]) for t in ("easy", "hard", "frontier")]
            log(f"{m:<30} {cells[0]:>10} {cells[1]:>10} {cells[2]:>10}")

    if frontier_rows:
        log("\nFRONTIER CHALLENGE BY CATEGORY  (the high-signal local adaptations)")
        frontier_header = (f"{'model':<30} " + " ".join(f"{c:>10}" for c in cats)
                           + f" {'overall':>10}")
        log(frontier_header)
        log("-" * len(frontier_header))
        for m in models:
            cells, all_scores = [], []
            for category in cats:
                scores = [float(r["score"]) for r in frontier_rows
            ...
            log(f"{m:<30} " + " ".join(f"{cell:>10}" for cell in cells)
                + f" {qcell(sum(all_scores), len(all_scores)):>10}")
    ...
    for m in models:
        ...
        if not fs:
            continue
        by_cat = {}
        for cat, tid, sc, _ in fs:
            by_cat.setdefault(cat, []).append(f"{tid} ({sc})")
        fail_lines.append(f"  {m}:")
        for cat in cats:
            if cat in by_cat:
                fail_lines.append(f"    {cat}: " + ", ".join(by_cat[cat]))
    if fail_lines:
        log("\nFAILURES  (score < 1.0)")
        for ln in fail_lines:
            log(ln)
    else:
        log("\nNo failures — every task scored 1.0. Run  --tier frontier  for more spread.")
    log(f"\nFull results: {csv_path}   Report: {out_dir / 'summary.md'}")

# The original report builder above is retained so old result directories can
# still be inspected while this file evolves. This later definition is the v2
# report path: it is state-aware, version-isolated, macro-category weighted, and
# exposes task-bootstrap uncertainty.
def write_summary(out_dir, log):
    csv_path = out_dir / "runs.csv"
    if not csv_path.exists():
        return
    ...
    for row in raw_rows:
        ...
        latest[(row["_suite_version"], row["model"], row["category"], row["task_id"])] = row
    ...
    def primary_scope(row):
        if selected_version == CALIBRATED_SUITE_VERSION:
            return (row.get("score_scope") or planned.get(
                (row["category"], row["task_id"]), {}
            ).get("score_scope")) in {"both", "complete_only"}
        return True
    ...
    # The legacy 45 score intentionally preserves its historical convention:
    # a completed/truncated response with a numeric grader result remains in
    # that score. Calibrated/full quality exclude non-quality states and expose
    # them through N/error counts instead.
    primary_source = numeric_rows if selected_version == LEGACY_SUITE_VERSION else quality_rows
    primary_rows = [row for row in primary_source if primary_scope(row)]
    models = sorted({row["model"] for row in all_rows})
    categories = CATEGORY_ORDER

    if planned:
        primary_planned = {(category, task_id) for (category, task_id), item in planned.items()
                           if selected_version != CALIBRATED_SUITE_VERSION
                           or item.get("score_scope") in {"both", "complete_only"}}
...
```""",
    ...
    s, _ = score_reasoning(nj, {"content": '{"server": {"name": "node-alpha", "cores": 16}, "status": "ok"}'})
    check("nested json exact", s == 1.0)
    s, _ = score_reasoning(nj, {"content": '{"server": {"name": "node-alpha", "cores": 17}, "status": "ok"}'})
...
def main():
    ...
    ap.add_argument("--profile", choices=["compact", "calibrated", "full"],
                    help="suite panel override; compact is legacy 45, calibrated runs 75 "
                         "requests and reports a validity-corrected 72-task score")
    ap.add_argument("--output-dir",
                    help="results directory override (compact defaults to results-compact-5x9)")
    ...
    ap.add_argument("--repeat", type=int, default=1, metavar="N",
                    help="run the panel N times with a different seed each time, so "
                         "per-task spread is measurable. Resumable: only the missing "
                         "attempts are run.")
    ...
    if args.profile:
        cfg["suite_profile"] = args.profile
    if args.output_dir:
        cfg["output_dir"] = args.output_dir
    elif args.profile == "compact":
        # Keep compact and exhaustive panels incomparable by construction:
        # an explicit compact run never appends to the full results history.
        cfg["output_dir"] = "results-compact-5x9"
    elif args.profile == "calibrated":
        cfg["output_dir"] = "results-calibrated-v2"
    ...
    profile = str(cfg.get("suite_profile", "full")).strip().lower()
    if profile not in {"compact", "calibrated", "full"}:
        sys.exit(f"invalid suite_profile {profile!r}")
    manifest = profile_task_manifest(profile, cfg)
    cfg["_suite_version"] = suite_version_for_profile(profile)
    ...
    if existing_csv.exists():
        with open(existing_csv, newline="") as handle:
            ...
            if "suite_version" in existing_header:
                ...
                sys.exit(
                    f"refusing to append versioned {profile!r} results to unversioned "
                    f"{existing_csv}. Use a fresh output_dir (the calibrated default is "
                    "results-calibrated-v2/)."
                )
    ...
    log(f"fleetbench: {len(models)} models × categories {categories} × "
        f"tier '{args.tier}' × profile '{cfg.get('suite_profile', 'full')}' → {out_dir}/")
    if repeats > 1:
        log(f"  {repeats} attempts per task, seeds {base_seed}..{base_seed + repeats - 1}")
    for replicate in range(repeats):
        ...
        for model_cfg in models:
            ...
            error = ""
            try:
                run_model(client, model_cfg, categories, tiers, cfg, out_dir, done, log)
            ...
            append_transcript(out_dir / "run_manifest.jsonl", {
                ...
                "benchmark_version": BENCHMARK_VERSION,
                "suite_version": cfg["_suite_version"], "profile": profile,
                "task_set_hash": cfg["_task_set_hash"],
