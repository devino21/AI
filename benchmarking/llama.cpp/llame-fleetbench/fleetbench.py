...
REASONING_TASKS = [
    ...
    },
    {
        ...
        "user": ("Reply with only this exact JSON object and nothing else: an object with key "
                 "\"server\" whose value is an object with \"name\" set to \"halo\" and \"cores\" "
                 "set to the number 24, and a top-level key \"status\" set to \"ok\"."),
        "expect": {"server": {"name": "halo", "cores": 24}, "status": "ok"},
    },
...
```""",
    ...
    s, _ = score_reasoning(nj, {"content": '{"server": {"name": "halo", "cores": 24}, "status": "ok"}'})
    check("nested json exact", s == 1.0)
    s, _ = score_reasoning(nj, {"content": '{"server": {"name": "halo", "cores": 25}, "status": "ok"}'})
