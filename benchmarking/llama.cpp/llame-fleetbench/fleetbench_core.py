...
# Bumped for the v3 panel: 16 of the 45 cells were swapped on measured item
# statistics, so v3 rows are NOT comparable to legacy-45 rows. The summary
# selects a single suite version and never blends two, which only works if a
# changed task set changes this string.
LEGACY_SUITE_VERSION = "fleetbench-panel-45-v3"
SUITE_PROFILE_NAME = "compact"
...
def collapse_replicates(scores: Iterable[float]) -> float | None:
    """Mean over independent attempts at one task.

    Replicates are repeated measurements of the SAME task, not extra tasks.
    Averaging them first is what keeps the task the unit of analysis; feeding
    every attempt into the category mean as though it were a distinct cell
    would shrink intervals by sqrt(k) while measuring nothing new.
    """
...
def paired_delta_interval(paired: Iterable[tuple[str, float, float]], *,
                          confidence: float = 0.95,
                          iterations: int = 4000,
                          seed: int = 4242) -> dict[str, Any]:
    """Category-stratified PAIRED bootstrap of the macro-score difference A-B.

    Comparing two independently-bootstrapped intervals and asking whether they
    overlap is the wrong test and badly underpowered: both models face the SAME
    tasks, so task difficulty is a shared nuisance term that cancels when the
    difference is taken per task. Resampling the per-task DIFFERENCE removes it.
    Non-overlapping intervals imply a significant delta, but the converse does
    not hold -- two 95% intervals can overlap substantially while the paired
    difference is comfortably clear of zero.

    `paired` yields (category, score_a, score_b) with one entry per task.
    """
    groups: dict[str, list[float]] = {}
    for category, a, b in paired:
        try:
            diff = float(a) - float(b)
        except (TypeError, ValueError):
            continue
        if math.isfinite(diff):
            groups.setdefault(str(category), []).append(diff)
    groups = {k: v for k, v in groups.items() if v}
    if not groups:
        return {"delta": None, "low": None, "high": None, "p_two_sided": None, "n": 0}

    observed = sum(sum(v) / len(v) for v in groups.values()) / len(groups)
    rng = random.Random(seed)
    draws = []
    for _ in range(max(200, int(iterations))):
        means = []
    ...
    draws.sort()
    alpha = (1 - confidence) / 2
    low, high = _percentile(draws, alpha), _percentile(draws, 1 - alpha)

    # Sign-flip permutation p-value: under the null that the two models are
    # exchangeable on each task, negating any per-task difference is equally
    # likely, so the null distribution is generated by random sign flips.
    perm = random.Random(seed + 1)
    hits = 0
    trials = max(200, int(iterations))
    for _ in range(trials):
        means = []
        for values in groups.values():
            flipped = [v if perm.random() < .5 else -v for v in values]
            means.append(sum(flipped) / len(flipped))
        if abs(sum(means) / len(means)) >= abs(observed) - 1e-12:
            hits += 1
    return {
        "delta": observed, "low": low, "high": high,
        "p_two_sided": hits / trials,
        "n": sum(len(v) for v in groups.values()),
    }


def variance_components(per_task_attempts: Mapping[Any, Iterable[float]]) -> dict[str, Any]:
    """Split observed spread into run-to-run noise and real task-to-task spread.

    Without replicates these are perfectly confounded, which is why a
    single-attempt suite cannot say whether a 2-point gap is a capability
    difference or resampling the same model twice. `within` is the pooled
    variance across attempts at one task (pure measurement noise); `between` is
    the variance of task means after that noise is removed.
    """
    task_means, within_sum, within_df = [], 0.0, 0
    replicated = 0
    for attempts in per_task_attempts.values():
        values = [float(v) for v in attempts
                  if isinstance(v, (int, float)) and math.isfinite(float(v))]
        if not values:
            continue
        ...
        if len(values) > 1:
            replicated += 1
            within_sum += sum((v - mean) ** 2 for v in values)
            within_df += len(values) - 1
    if not task_means:
        return {"within_sd": None, "between_sd": None, "replicated_tasks": 0,
                "noise_share": None}
    within_var = within_sum / within_df if within_df else None
    grand = sum(task_means) / len(task_means)
    observed_var = (sum((m - grand) ** 2 for m in task_means) / (len(task_means) - 1)
                    if len(task_means) > 1 else 0.0)
    between_var = (max(0.0, observed_var - (within_var or 0.0))
                   if within_var is not None else None)
    share = None
    if within_var is not None and observed_var > 0:
        share = min(1.0, within_var / observed_var)
    return {
        "within_sd": None if within_var is None else within_var ** .5,
        "between_sd": None if between_var is None else between_var ** .5,
        "replicated_tasks": replicated,
        "noise_share": share,
    }


def saturation_report(task_scores: Mapping[Any, Mapping[str, float]],
                      *, tolerance: float = 1e-9) -> dict[str, list]:
    """Classify tasks by how much they separate the models actually measured.

    A cell every model solves perfectly costs full wall-clock and contributes
    exactly zero to any ranking; it is a floor check, not a measurement. This
    reports them so compute can be moved to cells that discriminate.
    """
    dead, floored, live = [], [], []
    for key, by_model in sorted(task_scores.items()):
        values = [v for v in by_model.values()
                  if isinstance(v, (int, float)) and math.isfinite(float(v))]
        if len(values) < 2:
            continue
        spread = max(values) - min(values)
        if spread <= tolerance:
            (floored if all(v >= 1 - tolerance for v in values) else dead).append(key)
        else:
            live.append(key)
    return {"perfect_for_all": floored, "tied_not_perfect": dead, "discriminating": live}
