...
def _extract_objects(text):
    ...
    one object per subtask was therefore graded on the wrong object. A validation
    response had both subtasks numerically correct as sibling objects but scored
    0.0 because only the first object was examined.
    """
...
def _align_bundle_keys(value, expected):
    ...
    correct across multiple validation runs. Resolution now
...
def selftest_finance():
    ...
    # nested object. This reproduces a validation response that scored 0.0 while
    # being numerically correct on both subtasks. Structure is still charged,
    # but the work now counts.
    ...
    # used to score 0.0 on numerically correct answers. Ordinal-only keys and
